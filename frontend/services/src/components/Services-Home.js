...
//             ? await axios.get("/api/circulars/top-view/", {
//                 headers: {
...
//     try {
//       const response = await axios.get("/api/events/top-view/");
...
//                 }}
//               >
//                 {item.subject || item.event || item.title}
//               </div>
//               <div
//                 style={{
//                   fontSize: "0.9rem",
...
//                 }}
//               >
//                 {item.date || (item.from_date && `${item.from_date} - ${item.to_date}`)}
//               </div>
//             </a>
//           ))}
//         </div>
...
export default function UniversityHomepage() {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get("/api/events/top-view/");
      setEvents(response.data.events.slice(0, 8));
    } catch (error) {
      console.error("Error fetching events:", error);
    }
  };
