...
//   const [circularTypes, setCircularTypes] = useState(['Office Memorandums', 'Circulars', 'Notifications' ,'Office order']); // Example circular types
...
//     formDataToSubmit.append('c_type', formData.circularType); // Use c_type
//     formDataToSubmit.append('publish_id', formData.publishId); // Use publish_id
//     formDataToSubmit.append('date', formData.date);
//     formDataToSubmit.append('issued_by', formData.issuedBy); // Use issued_by
...
export default function Circular() {
  ...
  const [circularTypes] = useState(['Circulars', 'Notifications', 'Office Order']);
  ...
  const handleSubmit = async (e, action) => {
    ...
    if (validate()) {
        ...
        if (formData.file) {
            formDataToSubmit.append('file', formData.file); // Ensure file is valid
        }
  
        // Convert boolean to string
        formDataToSubmit.append('is_publish', action === 'Publish' ? 'true' : 'false');
        try {
            const response = await axios.post("/api/circular-data/", formDataToSubmit, {
                headers: {
                    'Content-Type': 'multipart/form-data',
            ...
            if (response.status === 200 || response.status === 201) {
                alert(response.data.message); // Use the message returned from the server
            ...
            if (error.response && error.response.data) {
                alert(error.response.data.message || 'There was an error submitting the form');
            } else {
